/*
该代码实现的是论文中V.RESULT部分用于比较的算法3：B.spheres，也是CTopPRM论文作者提出的算法
[3]R. Pˇeniˇcka and D. Scaramuzza, “Minimum-time quadrotor waypoint flight in cluttered environments,” IEEE Robot. Automat. Lett., vol. 7, no. 2, pp. 5719–5726, Apr. 2022.
机器人尚未解决由于在避免碰撞带来的非凸约束下最小化时间的挑战。而先前的工作依赖于简化的动力学或多项式轨迹表示，这些表示没有利用四旋翼飞行器的全部潜力，
论文的目的不是最小化时间。而是通过使用分层的、基于采样的方法和复杂的四旋翼模型来解决这个具有挑战性的问题
论文提出的方法首先在不同的拓扑结构中寻找路径，以指导后续的轨迹搜索，和动力学点质量模型的后续轨迹搜索
然后在点质量解的基础上，采用基于全四旋翼模型的渐近最优、基于动力学采样的方法，找到具有时间最优目标的可行轨迹
*/
...
class TopologicalPRM {
 public:
  ...
  bool isGateIntersection(const Vector<3> from, const Vector<3> to,
                          const Vector<3> gate, const Vector<3> gate_norm,
                          Scalar gate_radius, Scalar* dist_to_center);
  ...
  static void savePathsWithGateId(
  ...
  static std::vector<path_with_length<T>> removeTooLongPaths(
    std::vector<path_with_length<T>> paths);
  ...
  for (size_t fromi = 0; fromi < num_samples; ++fromi) {
    ...
    for (size_t nni = 1; nni < num_nn;
      ...
      if (from_node == start_ && constraint_start_) {
        ...
        const Vector<3> vec_from_start = to_node->data - start_->data;
        const double angle_from_start =
          angleBetween(vec_from_start, start_gate_x);
        if (angle_from_start > angle_limit_start_end_) {
          continue;
        }
      }
      ...
      if (to_node == end_ && constraint_end_) {
        const Vector<3> end_gate_x = endq_ * Vector<3>::UnitX();
        const Vector<3> vec_to_end = end_->data - from_node->data;
        // // INFO("constraint_end_ dot " << fromgate.dot(new_vec))
        // // INFO(to_node->data.transpose())
        if (to_node->data(0) == 6.0 && to_node->data(1) == 3.0) {
          // INFO("third gate " << vec_to_end.transpose().normalized())
          // INFO("end_gate_x " << end_gate_x.transpose().normalized())
          // INFO_VAR(angle_limit_start_end_)
        }
        const double angle_to_end = angleBetween(vec_to_end, end_gate_x);
        if (to_node->data(0) == 6.0 && to_node->data(1) == 3.0) {
        ...
        if (to_node->data(0) == 6.0 && to_node->data(1) == 3.0) {
          // INFO_GREEN("paaaaaaaaseeeeed")
        }
      }


      if (to_node != end_ and from_node != start_) {
        double dist_to_beg = 0;
        const Vector<3> start_gate_x = startq_ * Vector<3>::UnitX();
        bool intersection_start =
          isGateIntersection(from_node->data, to_node->data, start_->data,
                             start_gate_x, 1.9, &dist_to_beg);

        double dist_to_end = 0;
        const Vector<3> end_gate_x = endq_ * Vector<3>::UnitX();
        bool intersection_end =
          isGateIntersection(from_node->data, to_node->data, end_->data,
                             end_gate_x, 1.9, &dist_to_end);
...
//用于判断给定线段与gate是否相交，并返回相交结果以及交点到gate中心的距离
template<class T>
bool TopologicalPRM<T>::isGateIntersection(
  const Vector<3> from, const Vector<3> to, const Vector<3> gate,
  const Vector<3> gate_norm, Scalar gate_radius, Scalar* dist_to_center) {
  Vector<3> ray_direction = to - from;

  // compute gate norm vector
  // Vector<3> gate_norm = gates_[target_gate_id_]->getNorm();
  Scalar vec_cross_product = gate_norm.dot(ray_direction);
  if (fabs(vec_cross_product) < 1e-4) {
    // parallel, not intersection
    return false;
  }

  // might have intersection
  Vector<3> w = from - gate;
  Scalar s = -gate_norm.dot(w) / vec_cross_product;

  Vector<3> ps = from + s * ray_direction;

  // Scalar AB = ray_direction.norm();
  // Scalar AP = (quad_pos - ps).norm();
  // Scalar BP = (pre_quad_pos - ps).norm();

  *dist_to_center = (ps - gate).norm();
  if (s >= -0.00001 && s <= 1.00001 && *dist_to_center < gate_radius) {
    return true;
  } else {
    // no intersection
    return false;
...
采样孤立的一些点，并基于守卫节点将它们立即连接起来，
类似于raptor，但是会出现无法找到最短路径的问题
下面代码中的函数在raptor.hpp中几乎都出现过，而且类似，除了findDistinctPathsBlockingSpheres()这一个函数，位于754行
其他函数的解析请详见raptor.hpp
*/
template<class T>
...
template<class T>
void TopologicalPRM<T>::savePathsWithGateId(
  std::string filename, std::vector<path_with_length<T>> multigatepath) {
  // // INFO("save TopologicalPRM map to file " << filename);
  std::ofstream myfile;
  myfile.open(filename.c_str());
...
//找到连接两个点的不同路径，这些路径被球体阻挡，即球体碰撞检测不通过。路径的长度由其上的边权重之和确定，函数使用 Dijkstra 算法来计算最短路径。
template<class T>
          ...
          for (size_t pbi = 0; pbi < part_before.size(); pbi++) {
            ...
            if (part_before[pbi].plan.size() > 0) {
              if (deleted_nodes.count(part_before[pbi].to_id) == 0) {
                INFO_RED("path deleted_nodes should contain path end")
                INFO_VAR(part_before[pbi].to_id)
                exit(1);
              }
              ...
              if (!isPathCollisionFree(new_path).first) {
                INFO_RED("connected path is not collision free");
                auto tst = isPathCollisionFree(new_path);
                INFO_RED(tst.second.transpose())
                // exit(1);
                break;
              }
          ...
          for (size_t pai = 0; pai < part_after.size(); ++pai) {
            if (part_after[pai].plan.size() > 0) {
              if (new_starts.count(part_after[pai].from_id) == 0) {
                INFO_RED("path new_starts should contain path start")
...
TopologicalPRM<T>::find_geometrical_paths(
  ...
  for (size_t i = 0; i < topological_prms.size(); i++) {
    ...
    INFO_GREEN("algorithm time spheres " << elapsed_c.count() * 1e-9)
    ...
    shortened_paths =
      TopologicalPRM<Vector<3>>::removeTooLongPaths(shortened_paths);
    ...
    // INFO_GREEN("post processing time spheres " << elapsed.count() * 1e-9)
    // INFO("ended loop")
  }
